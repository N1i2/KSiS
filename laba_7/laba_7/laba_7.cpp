...
void Ping(const char* cHost, unsigned int Timeout=60, unsigned int RequestCount=10)
{
	...
	IP_OPTION_INFORMATION option = { 100, 0, 0, 0, 0 };

	for (unsigned int c = 0; c < RequestCount; c++)
	{
		int dwStatus = IcmpSendEcho(hIP,	//создаваемый файл
			ipaddr,							//IP
			SendData,						//буфер эхолокатера
			sizeof(SendData),				//размер 
			&option,						//опции эхозапроса
			pIpe,							//буффер эхоответа
			sizeof(ICMP_ECHO_REPLY) +		//
			sizeof(SendData),				//размер буфер ответа
			Timeout);
		if (dwStatus > 0)
		{
